...
接下来分别访问对应的请求`http://localhost:3000/user` 与 `http://localhost:3000/v2/user` 可以获取到如下的返回值：
...
`
完成上述操作之后开始检验是否配置正常。首先访问一个不存在的接口 `localhost:3000/test` ，此时可以对比自定义与原生的异常返回参数区别。
